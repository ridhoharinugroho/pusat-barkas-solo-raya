...
/**
 * 7. GOOGLE AUTHENTICATION & CREDENTIAL MANAGEMENT API
 */
...
export async function handleGoogleOneTapOrCredentialAPI() {
  if (typeof window !== 'undefined' && 'credentials' in navigator && navigator.credentials.get) {
    try {
      ...
      if (cred && (cred.id || cred.name)) {
        const selectedEmail = (cred.id || '').trim().toLowerCase();
        const selectedName = cred.name || selectedEmail.split('@')[0];
        return {
          success: true,
          email: selectedEmail,
          name: selectedName
      ...
      console.warn('[Credential API Notice]', e);
    }
  }
